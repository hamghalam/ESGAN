...
def DCGAN_Classifier(generator, discriminator_model,GAN_classifier, img_dim, patch_size, image_dim_ordering):   ##############################
    ...
    DCGAN_output = discriminator_model(list_gen_patch)
    added = Add()([generated_image, gen_input])
    classifier_output  = GAN_classifier(added)
    
    DCGAN = Model(inputs=[gen_input],
                  outputs=[generated_image, DCGAN_output, classifier_output],
                  name="DCGAN")

    return DCGAN


def Pereira_classifier(img_dim):
  ClassTrain=True
  num_classes=4
  classifier_input   = Input(shape = img_dim)

  conv1  = Conv2D(16,kernel_size=(3,3),padding='same',activation = 'relu',name='classconv1',trainable=ClassTrain)(classifier_input)
  conv2  = Conv2D(16,kernel_size=(3,3),padding='same',activation = 'relu',name='classconv2',trainable=ClassTrain)(conv1)
  conv3  = Conv2D(16,kernel_size=(3,3),padding='same',activation = 'relu',name='classconv3',trainable=ClassTrain)(conv2)
  pool1  = MaxPooling2D(pool_size=(3,3),strides=(2,2))(conv3)

  conv4  = Conv2D(32,kernel_size=(3,3),padding='same',activation = 'relu',name='classconv4',trainable=ClassTrain)(pool1)
  conv5  = Conv2D(32,kernel_size=(3,3),padding='same',activation = 'relu',name='classconv5',trainable=ClassTrain)(conv4)
  conv6  = Conv2D(32,kernel_size=(3,3),padding='same',activation = 'relu',name='classconv6',trainable=ClassTrain)(conv5)
  pool2  = MaxPooling2D(pool_size=(3,3),strides=(2,2))(conv6)

  flat   = Flatten()(pool2)
  dens1  = Dense(64, activation='relu',name='classdense1',trainable=ClassTrain)(flat)
  dens2  = Dense(64, activation='relu',name='classdense2',trainable=ClassTrain)(dens1)
  predictions = Dense(num_classes, activation='softmax',name='classdense3',trainable=ClassTrain)(dens2)

  classifier_model = Model(inputs=classifier_input, outputs=predictions, name = 'GANClassifier')
  classifier_model.summary()
  return classifier_model

def MyResNet18(img_dim):
   classifier_model=ResNet18(input_shape=img_dim,classes=4)
   classifier_model.summary()
   return classifier_model

def MyDensNet121(img_dim):
   classifier_model=model = DenseNetImageNet121(input_shape=img_dim,
                        bottleneck=True,
                        reduction=0.5,
                        dropout_rate=0.0,
                        weight_decay=1e-4,
...
def MyNASNetMobile(img_dim):
  classifier_model = NASNetMobile(input_shape=img_dim,
                 dropout=0.5,
                 weight_decay=4e-5,
